...
工具分类：
- 安全工具：search_car_rentals（只读查询）
- 敏感工具：book_car_rental, update_car_rental, cancel_car_rental（需要用户确认）
"""
...
SYSTEM_PROMPT = """你是专门处理租车预订的助手。
当用户需要帮助预订租车时，主助手会将任务委派给你。
根据用户偏好搜索可用租车，并向客户确认预订详情。
